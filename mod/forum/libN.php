...
4                 $DB->get_field_sql( $postcountsql.' AND fp.parent<>0',$postcountparams);
        if ($type==COMPLETION_AND) {
            $result = $result && $value;
        } else {
            $result = $result || $value;
    ...
    if($post->mailed==42)
	 {
		...
		/*if(! mysql_query($sql))
		{
			print("Query $sql failed!<br>\n".mysql_error()."<br>\n");
		}
		#	print($sql.", line 3923, lib.php\n");
		$sql="UPDATE {$CFG->prefix}forum_posts SET mailed=0 WHERE id=".$post->id."";
		execute_sql($sql);*/
