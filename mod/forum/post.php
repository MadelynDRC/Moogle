...
if (forum_is_subscribed($USER->id, $forum->id)) {
	...
	/**
	*  Moodle-Listo: In the case of a moderated message, the original message should be shown
	* and not the "This message is being moderated..." replacement.
	* */
	
	
	 if(isset($post->mailed) and $post->mailed == 42)
	 {
		//$query="SELECT * FROM trans_forum_posts WHERE id=".$post->id.";";
		//$sth=mysql_query($query);
		//$row=mysql_fetch_assoc($sth);
		$row = $DB->get_record('trans_forum_posts', array('id' => $post->id));
       
	 	$post->message=$row->message;
	 	$post->subject=$row->subject;
	
	
		/** This database update existed only for debugging purposes. 
		* $query="UPDATE forum_posts SET moderated=1 WHERE id=".$post->id.";";
		*print($query.", line 761, post.php\n");
		**/
...
if ($fromform = $mform_post->get_data()) {
    ...
    //$fromform->mailed =empty($fromform->skipsinglemoderation) ? 0 : 1;
    ...
    if ($fromform->edit) {           // Updating a post
        ...
        /** Moodle-Listo: "mailed" contains the status of moderation. 
         *  We need to transmit it, to trigger translations and give feedback 
         *  about the translating status for a few seconds.
         * */
		$updatepost->mailed = $realpost->mailed;
