...
/** Moodle-listo: include requerido para la traducción y moderación de los foros **/
require_once('liblisto.php');
/**/
...
function forum_cron() {
	...
	/**
	 * Moodle-Listo:  moderation_workflow(), llama la función que dispara el proceso de moderación y
	 * traducción de foros en Moodle.
	 * * */
	 try {
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                if (!$mailresult){
                 ...
                 /* Moodle-Listo
                  * Se necesita un log con los correos enviados de forma exitosa también
                  * */  
				 add_to_log($course->id, 'forum', 'mail enviado', "discuss.php?d=$discussion->id#p$post->id",
                               substr(format_string($post->subject,true),0,30), $cm->id, $userto->id);
    ...
    if ($posts) {
        foreach ($posts as $post) {
            ...
            /**
             * Moodle-Listo:  Log de mensajes enviados
             * */
            //add_to_log($mailcount[$post->id], 'forum', 'users were sent post', '', '', $post->id, $post->subject);
...
function forum_make_mail_post($course, $cm, $forum, $discussion, $post, $userfrom, $userto,
	...
	/**
	 * Moodle-Listo: Translated messages
	 * Preparing the exchange of the translated reply link by a link
	 * which leads to the "multilanguage" forum, to make that process
	...
	$row = $DB->get_record('forum_post_id_translated', array('target_id' => $post->id));
	        
	if(isset($row->orig_id))
	{
		/** Moodle-Listo: place the link by a reply link to the original message in
		* the multilanguage forum
		* */
		$original_id_of_translated_post=$row->orig_id;
	}
	else
	{
		/** Moodle-Listo: This is no translated message, or cron.php did not start
		* after the original post, so let the link as it is (for now).
		* */
		$original_id_of_translated_post=$post->id;
	}
    if (!isset($userto->viewfullnames[$forum->id])) {
        $viewfullnames = has_capability('moodle/site:viewfullnames', $modcontext, $userto->id);
    } else {
...
// Commands
    ...
    if ($reply) {
		/**
		 * Moodle-Listo: Reply only in main post.  Changed .$post->id. by .$original_id_of_translated_post
         **/
        //$commands[] = '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forum/post.php?reply='.$post->id.'">'.
		$commands[] = '<a target="_blank" href="'.$CFG->wwwroot.'/mod/forum/post.php?reply='.$original_id_of_translated_post.'">'.get_string('reply', 'forum').'</a>';
    }
 ...
 * @return void
 * Moodle-Listo: In this function we must change reply links in post translateds
 */
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
	...
	/*$query=
		"SELECT orig_id 
		FROM {$CFG->prefix}forum_post_id_translated 
			WHERE 
				target_id=".$post->id.";";
	$sth=mysql_query($query);
	$row=mysql_fetch_assoc($sth);*/
	...
	if(isset($row->orig_id))
	{
		/** Replace the link by a reply link to the original message in
		* the multilanguage forum
		**/
		$original_id_of_translated_post=$row->orig_id;
	}
	...
	{
		/** This is no translated message, or cron.php did not start
		* after the original post, so let the link as it is (for now).
		**/
		$original_id_of_translated_post=$post->id;
	}
   ...
   if ($reply) {
        ...
        $commands[] = array('url'=>new moodle_url('/mod/forum/post.php#mformforum', array('reply'=>$original_id_of_translated_post)), 'text'=>$str->reply);
        //$commands[] = '<a href="'.$CFG->wwwroot.'/mod/forum/post.php?reply='.$original_id_of_translated_post.'">'.$strreply.'</a>';
...
function forum_update_post($post, $mform, &$message) {
	...
	/**
     * Moodle-Listo
     * We need to set the "mailed" attribute again to zero, after moderation.
	 * */
	    
    if($post->mailed==42) 
	 {
	    /* 
	     * Traducciones lanzadas si solo todo el proceso es correcto de lo contrario 
	     * el mensaje se queda en estado de no moderado
	     * */
	 	try {
			$transaction = $DB->start_delegated_transaction();
			// Perform some $DB stuff
			
			//$sql="UPDATE {$CFG->prefix}trans_forum_posts SET moderated=1 WHERE id=".$post->id.";";
			
			$skipsinglemoderation = '[SKIPSINGLEMODERATION]';
			$skipsinglemoderation = str_replace($skipsinglemoderation, "",  $post->message);
						
			$record = new stdClass();
			$record->id      = $post->id;
			$record->moderated  = 1;
			
			if ($post->message != $skipsinglemoderation) {
				$record->singlemoderation  = 0;
			}
			$DB->update_record("trans_forum_posts", $record);
			...
			$record->id      = $post->id;
			$record->mailed  = 0;
			$record->message = $skipsinglemoderation;
			
			
			$DB->update_record("forum_posts", $record);

			forum_translate_post($post->id);
		     
		    $transaction->allow_commit();
		    
		  } catch (Exception $e) {
				print "Error!!! \n Proceso de traducción no completado mensaje permanece sin traducirse. Favor intentar de nuevo. ";
				$transaction->rollback($e); // rethrows exception
		  }
